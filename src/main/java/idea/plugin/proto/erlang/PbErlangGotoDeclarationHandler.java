 ...
 */
package idea.plugin.proto.erlang;
...
public final class PbErlangGotoDeclarationHandler implements GotoDeclarationHandler {
  ...
  private static ImmutableCollection<? extends PbElement> findPbByIndex(QualifiedName protoName, Project project) {
    ...
    if (pbNamedElements.size() > 0) {
      HashSet<PbElement> result = new HashSet<>();
      for (PbNamedElement element:pbNamedElements){
        Collection<? extends PbElement> pbElements = locateSymbolInProtoFile(element.getPbFile(), protoName);
        result.addAll(pbElements);
      }
      return ImmutableList.copyOf(result);
    }
    return null;
  }
  private static Collection<? extends PbElement> locateSymbolInProtoFile(
    ...
    return fileSymbols.get(qualifiedName);
  }
}
